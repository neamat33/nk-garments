        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Production Send List</h4>
                </div>
                <div class="row">
                    <div class="card-body">
                                ...
                                <tbody>
                                    @foreach ($productions as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $item->items->sum('qty') }} Pcs</td>
                                            <td class="text-center">{{ $item->items->sum('weight') }} Kg</td>
                                            <td>{{ $item->note ?? 'N/A' }}</td>

                                            <td>
                                                <a href="{{ route('production_send.show', $item->id) }}"
                                                    class="btn-sm btn-info"><i class="fa fa-eye" title="View"></i></a>
                                                <a href="{{ route('production_send.print', $item->id) }}"

<script>
    $(document).ready(function() {
        ...
        $('#addrow1').on('click', function() {
            ...
            cols +='<td><input type="number" name="dozen[]" class="form-control form-control-sm dozen"></td>';
            cols +=
                '<td><input type="number" name="qty[]" class="form-control form-control-sm qty"></td>';
            cols +=
                '<td><input type="number" name="weight[]" class="form-control form-control-sm weight"></td>';
            cols +=
                '<td><button class="btn bg-gradient-danger deleteRow"><span class="fa fa-remove"></span></button></td>';
            newRow.append(cols);
            $('table.mytable1').append(newRow);
            $(".select1").select2();
        });

        $(".mytable1").on("change", '.item-select', function() {
            var selectedOption = $(this).find(':selected');
            var itemDetails = selectedOption.data('details');
            var stockValue = selectedOption.data('stock');
            var itemId = selectedOption.val();

            var addButton = $('#addrow1');
        ...
        function updateVariationDropdown(variations) {
            ...
            var variationDropdown = $(".mytable1").find('.variation').last();
            ...
            variations.forEach(function(variation) {
                ...
                variationDropdown.append('<option value="' + variation.id + '">' + variationName +
                    '</option>');
            });
        ...
        $(".mytable1").on("keyup",
            'input[class*="rate"], input[class*="main_unit_qty"], input[class*="sub_unit_qty"]',
            function(event) {
                calculateRow($(this).closest("tr"));
                calculateGrandTotal();
            });

        $(".mytable1").on("click", "button.deleteRow", function(event) {
        ...
        function calculateRow(row) {
            ...
            var sub_unit_qty = +row.find('input[class*="sub_unit_qty"]').val() / related_unit ||
            0; // If sub_unit_qty is empty, default to 0
            row.find('input[class*="sub_total"]').val((rate * (main_unit_qty + sub_unit_qty)).toFixed(2));
        }

        function calculateGrandTotal() {
            var grandTotal = 0;
            $(".mytable1").find('input[class*="sub_total"]').each(function() {
                grandTotal += +$(this).val();
            });
    ...
    function getSendInfo() {
        ...
        $.ajax({
            ...
            success: function(res) {
                if(res.status == 3){
                    $("#production_id").val('');
                    swal("The Production Items have been received!", "", "warning");
                }else if(res.status == false){
                    $("#production_id").val('');
                    swal("The Production ID is unavailable!", "", "warning");
                }
                $('#send_item').html(res.view);
            }
        });

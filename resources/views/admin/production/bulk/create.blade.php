...
<div class="content-body" style="min-height: 500px">
    <div class="container-fluid">
        <div class="card-header mb-3">
            <h4 class="card-title">Create Bulk Send </h4>
        </div>
        <div class="basic-form">
            <form method="POST" action="{{ route('bulk_send.store') }}">
                @csrf
                <div class="row">
                    <div class="col-6">
                        <div class="mb-3 row">
                            <label class="col-sm-4 col-form-label">Department From<span class="text-danger">*</span> :</label>
                            <div class="col-sm-8">
                                <select name="department_from" id="" class="form-control form-control-sm" required>
                                    @foreach($departments as $dep)
                                        <option @if($department_id == $dep->id) selected @endif value="{{$dep->id}}">{{$dep->name}}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('department_from'))
                                    <span class="invalid-feedback">{{ $errors->first('department_from') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-8">
                                <select name="department_to" id="" class="form-control form-control-sm" required>
                                    ...
                                    @foreach($departments as $dep)
                                        <option @if($dep->id==2) selected @endif value="{{$dep->id}}">{{$dep->name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <table class="table table-responsive-md mytable">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width:15%"><strong> Cone </strong></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td>
                                            <select name="new_item[]" id="select2-dropdown" class="item-select">
                                                ...
                                                @php $rawStock = $val->raw_stock(); @endphp
                                                    <option value="{{ $val->id }}" data-details="{{ "Name: " . $val->name . ", Type: " . $val->type}}" data-qty="{{ $rawStock['qty'] }}" data-cone="{{ $rawStock['cone'] }}" data-mainunit-name="{{ $val->main_unit->name }}" data-subunit-name="{{ $val->sub_unit_name->name }}" data-subunit-related="{{ $val->unit_related_by->related_by }}" data-price="{{ $val->unit_price }}">{{ $val->name }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <td>
                                            <input type="number" name="qty[]" id="qty" placeholder="Kg" class="form-control form-control-sm qty">
                                        </td>
                                        <td>
                                            <input type="number" name="cone[]" id="cone" placeholder="Pcs" class="form-control form-control-sm cone">
                                        </td>

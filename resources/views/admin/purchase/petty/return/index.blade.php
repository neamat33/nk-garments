    ...
    <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="purchase_form" autocomplete="off" value="{{ request('purchase_form') }}">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label class="form-label"><strong>Phone</strong></label>
                                <input class="form-control" type="text" name="phone" autocomplete="off" value="{{ request('phone') }}">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label class="form-label"><strong>Purchase Date</strong></label>
                                <input class="form-control input-daterange-datepicker" type="text" name="purchase_form" autocomplete="off" value="{{ request('purchase_form') }}">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label class="form-label"><strong>Return Date</strong></label>
                                <input class="form-control input-daterange-datepicker" type="text" name="return_date" autocomplete="off" value="{{ request('return_date') }}">
                            </div>
                        </div> 
        ...
        <div class="card print" id="print-area">
            <div class="card-header">
                <h4 class="card-title">Petty Purchase Return List</h4>
            </div>
            <div class="row">
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-responsive-md">
                            ...
                            <tbody>
                                ...
                                <tr>
                                   <td><strong>{{$purchase_return->id}}</strong></td>
                                   <td>{{$purchase_return->purchase_id}}</td>
                                   <td>{{$purchase_return->purchase_form}}</td>
                                    <td>{{$purchase_return->phone}}</td>
                                   <td>{{date('d M, Y', strtotime($purchase_return->purchase_date))}}</td>
                                   <td>{{date('d M, Y', strtotime($purchase_return->return_date))}}</td>
                                   <td>
                                        <ul>
                                             @foreach($purchase_return->items as $item)
                                             <li>{{$item->items->name}} @if($item->variation) - V : {{ $item->variation->name }}@endif * {{ $item->qty}}</li>
                                             @endforeach
                                        </ul>
                                   </td>
                                    <td>{{$purchase_return->note}}</td>
                                   <td class="print_hidden">
                                       <div class="btn-group" role="group">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                <a class="dropdown-item" href="" data-bs-toggle="modal" data-bs-target=".delete-modal" onclick="handle({{$purchase_return->id}})"><i class="fas fa-trash"></i> Delete</a>
                                                @endcan
                                            </div>
                                        </div>
                                   </td>
                        ...
                        {!! $purchase_returns->appends(Request::except('_token'))->links() !!}
                    </div>
                </div>
            </div>
        </div>
...
<script>
    //Delete Code
    function handle(id) {
       var url = "{{ route('purchase-return.destroy', 'purchase_id') }}".replace('purchase_id', id);
